...
class _WeatherOverviewScreenState extends State<WeatherOverviewScreen> {
  ...
  @override
  void initState() {
    Future.delayed(Duration.zero).then((_) =>Provider.of<Location>(context,listen: false).combineAllData() );
    setState(() {
     _isInit=false;
    });
    super.initState();
  ...
  Widget build(BuildContext context) {
    ...
    return SafeArea(
      child: Scaffold(
        body: _isInit? Center(child: CircularProgressIndicator(),) :SingleChildScrollView(
          child: Column(
            children: [
              Container(
